...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
		...
		if (isUnix()) {
            ...
            rc = bat returnStatus: true, script: "\"C:\\Program Files\\sfdx\\bin\\sfdx\" force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"c:\\openssl\\bin\\server.key\" -d --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
        }
